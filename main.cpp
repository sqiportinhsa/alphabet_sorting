...
int main() {
    struct stat a = {};
    stat("onegin.txt", &a);
    int amount_of_symbols = a.st_size;
    ...
    for (int i = 0; i < amount_of_symbols && (c = getc(input)) != EOF; ++i) {
        onegin[i] = (char) c;
        if (c == '\n') {
            ++c;
        }
    ...
    for (int n_sym = 0; n_sym < amount_of_symbols; ++n_sym) {
        if (onegin[n_sym] == '\n') {
    ...
    printf("%c", pointers_to_strings[1][1]);
